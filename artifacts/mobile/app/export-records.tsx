...
export default function ExportRecordsScreen() {
  ...
  const generateTextReport = () => {
    ...
    lines.push(divider);
    lines.push("  NAVIMED HEALTH SUMMARY REPORT");
    ...
    lines.push(divider);
    lines.push("  This report was generated by NaviMED");
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        a.href = url;
        a.download = `NaviMED_Health_Report_${new Date().toISOString().split("T")[0]}.txt`;
        a.click();
        URL.revokeObjectURL(url);
      } else {
        await Share.share({
          message: report,
          title: "NaviMED Health Report",
        });
      }
    } catch (e) {
